...
- The workers will run the deployments, and can add or remove dynamically. 

By default the `controller` machine will also be a `worker` machine, thus one machine can form a single node cluster.
